...
Provide an Igniter installer for the `phoenix_prodmin` package that, when run
...
- Compose cleanly with the standard Igniter workflow so the installer can be invoked as part of `mix igniter.new ... --install phoenix_prodmin`.
...
- The package exposes an Igniter installer task discoverable as the `--install phoenix_prodmin` target.
...
- **Architecture:** the package's core is an Igniter installer module that registers itself as the entry point for `--install phoenix_prodmin`.
...
- Running `mix igniter.new myapp --with phx.new --with-args="--no-ecto --no-email" --install phoenix_prodmin` produces a project where `MIX_ENV=prod mix phx.server` succeeds with no further file edits.
